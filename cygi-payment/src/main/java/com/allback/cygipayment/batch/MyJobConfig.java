...
public class MyJobConfig {
    ...
    public JpaPagingItemReader<Reservation> reservationReader() {
        ...
        parameters.put("status", "예약완료");
        parameters.put("concertIds", concertList);
        return new JpaPagingItemReaderBuilder<Reservation>()
            .name("reservationReader")
            .entityManagerFactory(entityManagerFactory)
            .pageSize(chunkSize)
            .queryString(
                "SELECT r FROM Reservation r WHERE r.status like :status AND r.concertId IN :concertIds")
